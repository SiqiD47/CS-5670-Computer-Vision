...
class AnimalBaselineNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = self.ReLU(x)
        # resize x to [batch size x input size to linear layer]
        x = x.view(-1, 1536)
        # print(x.size())
        x = self.fc(x)
        x = self.ReLU(x)
...
class Shift(object):
  ...
  Shifts input image by random x amount between [-max_shift, max_shift]
    ...
    def __call__(self, image):
        ...
        image = np.asarray([cv2.warpAffine(image[i], M, (H, W)) for i in range(_)]).reshape(_, H, W)
...
class Contrast(object):
    ...
    def __call__(self, image):
        ...
        if contrast == 0:
          image = np.full((_, H, W), np.mean(image))
        else:
...
class HorizontalFlip(object):
    ...
    def __call__(self, image):
        ...
        prob = random.random()
        if prob <= self.p:
          image = torch.from_numpy(np.flip(image,axis=2).copy())
...
def get_adversarial(img, output, label, net, criterion, epsilon):
    ...
    grad_np = img.grad.numpy()
    img_np = img.detach().numpy()

    perturbed_image_np = np.clip(img_np + np.sign(grad_np) * epsilon,0,1)
    noise_np = perturbed_image_np - img_np

    perturbed_image = torch.from_numpy(perturbed_image_np).type(torch.FloatTensor)
